export class TableMatrixHandler {
    ...
    getAdjustedRows(length) {
        return this.uniqueElementsMatrix()
            .map((r) =>
                r.length == length ? r : this.labelRow(length, r))
            .filter(r => r[0].current != null).map((r) =>
                r.map((c) => c.current.innerText));
    }
    ...
    makeElementFrom(previous, position) {
        const VALID_ATTRIBUTES = ['data-rowtype', 'data-spanselection', 'type', 'data-selectable', 'class', 'style'];
