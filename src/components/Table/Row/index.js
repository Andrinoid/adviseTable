    ...
    ({
        ...
        useLayoutEffect(() => {
            let index = Array.prototype.indexOf.call(
                document.querySelectorAll(`.${tableId}-tableRow`),
        ...
        const childrenWithProps = React.Children.map(validChildren, (child) => {
            ...
            if (React.isValidElement(child)) {
                ...
                if (i === 0) {
                    ...
                    colType = "middle";
                    left =
                        leftOffset + firstColWidth + (numCols - 1) * colWidth;
                    if (colspan) {
                        width =
                            colspan * colWidth + lastColPaddingLeft || "auto";
                    } else {
                        width = colWidth;
                    }
                }
        ...
        return (
            <>
                ...
                >
                    {menuContent && (
                        <RowMenu tableId={tableId}>{menuContent}</RowMenu>
                    )}
